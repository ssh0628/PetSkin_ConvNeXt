...
class ThreeCropCSVDataset(Dataset):
    ...
    Later, DP(x) is built as:
      [prob_roi_model(roi_img),
       prob_ext_model(ext_img),
       prob_off_model(off_img),
       prob_3view_model(mean_logits(roi_img, ext_img, off_img))]

    Final DP shape is [L=4, C=8].
    """
...
def main():
    ap = argparse.ArgumentParser(description="Build Decision Templates from 4 models: roi/ext/off/3view-joint.")
    ...
    ap.add_argument("--save_dt_path", default="/root/project/Result/DT/4models_WCE_DT", type=str, help="output .npy path, DT shape [8,4,8]")
    ...
    dt_sum = np.zeros((args.num_classes, 4, args.num_classes), dtype=np.float64)
