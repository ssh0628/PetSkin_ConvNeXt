...
class ThreeViewCSVDataset(Dataset):
    """
    Returns [3, C_in, H, W] image tensor stack for one sample.
    DP(x) is built from model probabilities over these 3 views:
      DP(x) shape = [L=3, C=8]
    """
...
def compute_metric(dt_i: np.ndarray, dp_x: np.ndarray, metric: str) -> float:
    """
    DT_i and DP(x) both have shape [3, 8].
    Implemented metrics from the paper family, excluding point-wise S4/I4/I5/C.
...
def main():
    ap = argparse.ArgumentParser(description="Evaluate a single 3-view ConvNeXt model with Decision Template fusion.")
    ap.add_argument("--ckpt_path", default="/root/project/Result/multi_roi_trained/3view(independent)+WCE_ver3/best_model.pth", type=str)
    ap.add_argument("--dt_path", default="/root/project/Result/DT/WCE_ver3_DT.npy", type=str, help="DT .npy created by dt_build_single_model.py")
    ...
    ap.add_argument("--out_dir", default="/root/project/Result/multi_roi_inference/DT/WCE_ver3_DT/s2", type=str)
    ap.add_argument("--save_pred_csv", default="/root/project/Result/multi_roi_inference/DT/WCE_ver3_DT/s2/pred.csv", type=str)
    ...
    assert dt_path.exists(), f"dt not found: {dt_path}"
    ...
    dt = np.load(dt_path)
    if dt.shape != (args.num_classes, 3, args.num_classes):
        raise ValueError(f"DT shape must be ({args.num_classes}, 3, {args.num_classes}), got {dt.shape}")
    ...
    with torch.no_grad():
        for views, y, sample_ids in tqdm(loader, desc="DT Eval"):
            batch_probs = []
            for v in range(3):
            ...
            dp_batch = np.stack(batch_probs, axis=1)  # [B, 3, 8]
            y_np = y.numpy()
