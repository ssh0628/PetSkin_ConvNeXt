...
def main():
    ap = argparse.ArgumentParser(description="Evaluate DT fusion from 2 three-view models.")
    ...
    ap.add_argument("--dt_path", required=True, type=str)
    ...
    ap.add_argument("--save_pred_csv", default="", type=str)
    ...
    for path_obj, name in [(ckpt_a, "ckpt_a"), (ckpt_b, "ckpt_b"), (dt_path, "dt_path"), (npy_dir, "npy_dir")]:
    ...
    dt = np.load(dt_path)
    if dt.shape != (args.num_classes, 2, args.num_classes):
        raise ValueError(f"DT shape must be ({args.num_classes}, 2, {args.num_classes}), got {dt.shape}")
