...
class ThreeCropDataset(Dataset):
    """
    Each sample returns roi/ext/off crops once.

    DP(x) is built as [8, 8] in this order:
      0: roi_a on ROI
      1: roi_b on ROI
      2: ext_a on EXT
      3: ext_b on EXT
...
def main():
    ap = argparse.ArgumentParser(description="Build Decision Templates from 8 models: roi/ext/off/3view x 2 seeds.")
    ...
    ap.add_argument("--save_dt_path", default="/root/project/Result/DT/8models_WCE_DT", type=str, help="output .npy path, DT shape [8,8,8]")
    ...
    dt_sum = np.zeros((args.num_classes, 8, args.num_classes), dtype=np.float64)
