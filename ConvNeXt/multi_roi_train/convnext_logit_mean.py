...
SAVE_DIR = "/root/project/convnext/convnext_sqrt_roi_2"
...
class NPYPathDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if USE_OFF_VIEW:
            return (roi_img, ext_img, off_img), label, idx
        return (roi_img, ext_img), label, idx
...
for epoch in range(NUM_EPOCHS):
    ...
    for views, y, idx in tqdm(train_loader, desc="Train"):
        if USE_CHANNELS_LAST:
            roi_x = views[0].to(device, non_blocking=True, memory_format=torch.channels_last)
            ext_x = views[1].to(device, non_blocking=True, memory_format=torch.channels_last)
            off_x = views[2].to(device, non_blocking=True, memory_format=torch.channels_last) if USE_OFF_VIEW else None
        else:
            roi_x = views[0].to(device, non_blocking=True)
            ext_x = views[1].to(device, non_blocking=True)
            off_x = views[2].to(device, non_blocking=True) if USE_OFF_VIEW else None
        ...
        with torch.amp.autocast(device_type=amp_device, dtype=amp_dtype):
            ...
            if USE_OFF_VIEW:
                out_off = model(off_x)
                loss = (criterion(out_roi, y) + criterion(out_ext, y) + criterion(out_off, y)) / 3.0
                out = (out_roi + out_ext + out_off) / 3.0
            else:
                loss = (criterion(out_roi, y) + criterion(out_ext, y)) / 2.0
                out = (out_roi + out_ext) / 2.0
    ...
    with torch.no_grad():
        for views, y, idx in tqdm(val_loader, desc="Val"):
            if USE_CHANNELS_LAST:
                roi_x = views[0].to(device, non_blocking=True, memory_format=torch.channels_last)
                ext_x = views[1].to(device, non_blocking=True, memory_format=torch.channels_last)
                off_x = views[2].to(device, non_blocking=True, memory_format=torch.channels_last) if USE_OFF_VIEW else None
...
correct, total = 0, 0
with torch.no_grad():
    for views, y, idx in tqdm(test_loader, desc="Test"):
        if USE_CHANNELS_LAST:
            roi_x = views[0].to(device, non_blocking=True, memory_format=torch.channels_last)
            ext_x = views[1].to(device, non_blocking=True, memory_format=torch.channels_last)
            off_x = views[2].to(device, non_blocking=True, memory_format=torch.channels_last) if USE_OFF_VIEW else None
