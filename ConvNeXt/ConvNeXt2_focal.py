...
# -------------------------
FOCAL_GAMMA = 0.0
FOCAL_ALPHA = None  # set to float or list/np array per class
FOCAL_REDUCTION = "mean"  # "mean" or "sum"
USE_AUTO_ALPHA = True
ALPHA_MODE = "inv_sqrt"  # "inv" or "inv_sqrt"
...
def focal_loss(logits, targets, gamma=2.0, alpha=None, reduction="mean"):
    ...
    if alpha is not None:
        ...
        if alpha_t.numel() == 1:
            ...
            loss = alpha_t.gather(0, targets) * loss
...
def compute_alpha_from_labels(labels: np.ndarray, num_classes: int):
    ...
    return alpha.tolist(), counts.tolist()
...
SAVE_DIR = "/root/project/convnext/convnext_sqrt_CE"
...
if USE_AUTO_ALPHA:
    ...
    if train_labels_path.exists():
        ...
        FOCAL_ALPHA, _counts = compute_alpha_from_labels(train_labels, NUM_CLASSES)
        print(f"[Info] Auto alpha enabled ({ALPHA_MODE}).")
    else:
        print(f"[WARN] train_labels.npy not found for auto alpha: {train_labels_path}")
    ...
    def __getitem__(self, idx):
        ...
        try:
            with Image.open(path) as img:
                ...
                if roi_box:
                    img = img.crop(roi_box).resize((IMG_SIZE, IMG_SIZE), resample=Image.BICUBIC)
...
for epoch in range(NUM_EPOCHS):
    ...
    for x, y, idx in tqdm(train_loader, desc="Train"):
        ...
        with torch.amp.autocast(device_type=amp_device, dtype=amp_dtype):
            out = model(x)
            loss = focal_loss(out, y, gamma=FOCAL_GAMMA, alpha=FOCAL_ALPHA, reduction=FOCAL_REDUCTION)
